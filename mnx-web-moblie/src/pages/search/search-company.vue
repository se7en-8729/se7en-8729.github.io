...
export default {
  ...
  created() {
    // 获取搜索记录
    if (localStorage.getItem('searchCompanyHistory')) {
      this.historyList = JSON.parse(localStorage.getItem('searchCompanyHistory'))
    }
  },
  mounted() {
    // 让input获得焦点
  ...
  methods: {
    search() {
      ...
      if (this.searchContent) {
        if (localStorage.getItem('searchCompanyHistory')) {
          // 已经存在搜索记录
          let arr = JSON.parse(localStorage.getItem('searchCompanyHistory'))
          let index = arr.indexOf(this.searchContent)
          if (index !== -1) { arr.splice(index, 1) }
          arr.unshift(this.searchContent)
          localStorage.setItem('searchCompanyHistory', JSON.stringify(arr))
        } else {
          // 不存在搜索记录
      ...
      this.$root.searchCompany = this.searchContent
      // 在还没创建jobs组件的情况下像vue根的全局变量写入一个搜索值
      this.$root.eventHub.$emit('changeTabBar', 2)
      this.$root.eventHub.$emit('searchCompany', this.searchContent)
    ...
    deleteItem(index) {
      ...
      localStorage.setItem('searchCompanyHistory', JSON.stringify(this.historyList))
    },
    clearAll() {
      this.historyList = []
      localStorage.setItem('searchCompanyHistory', '')
    }
  }
}
</script>
