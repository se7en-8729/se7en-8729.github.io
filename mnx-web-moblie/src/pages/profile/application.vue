<template>
  <div class="view resume">
    ...
    <div id="mobile-neitui-resume" v-show="showPage==='resume'">
      ...
      <button class="completeBtn applyBtn" :class="{disabled: isApply}" @click="apply">确定投递</button>
    </div>
    <!-- 基本信息编辑 -->
      ...
      <div class="resume-detail-content">
        <div class="resume-detail-view">
          <group class="delBorder">
            ...
            <datetime class="necessary" :class="{fixText: hasStart}" v-model="start_date" format="YYYY-MM" title="入学时间" :min-year="2000" :end-date="end_date" confirm-text="完成" cancel-text="取消" placeholder="请选择入学时间" @on-change="startChange"></datetime>
            <datetime class="necessary" :class="{fixText: hasEnd}" v-model="end_date" format="YYYY-MM" title="毕业时间" :min-year="2000" :start-date="start_date" confirm-text="完成" cancel-text="取消" placeholder="请选择毕业时间" @on-change="endChange"></datetime>
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            if (!this.resumeData.basic.name || !this.resumeData.basic.mobile || !this.resumeData.basic.email || !this.resumeData.education.length || !this.resumeData.work || !this.resumeData.practices) {
              this.isApply = true
              this.$vux.toast.text('请补全申请表必填信息')
            }
          }
        })
    },
    saveData() {
      let obj = {
        ...
        webBasic: {
          ...
          mobile: this.resumeData.basic.mobile,
          email: this.resumeData.basic.email,
          gender: this.resumeData.basic.gender,
          birthday: this.resumeData.basic.birthday,
          province: this.resumeData.basic.city,
          city: this.resumeData.basic.currentCity
        },
        education: this.resumeData.education,
        work: this.resumeData.work,
        practices: this.resumeData.practices,
        club: this.resumeData.club,
        reward: this.resumeData.reward,
        skills: this.resumeData.skill,
        certificate: this.resumeData.certificates,
        language: this.resumeData.language
      }
      if (this.resumeData.basic.name && this.resumeData.basic.mobile && this.resumeData.basic.email && this.resumeData.education.length && this.resumeData.work && this.resumeData.practices) {
        this.isApply = false
      } else {
      ...
      console.log(obj)
      this.$http.post(`/user/saveReusme?appId=${this.$route.query.appId}`, obj)
        .then((res) => {
          console.log(res)
        })
    },
