  ...
  <div class="view resume">
    ...
    <div id="mobile-neitui-resume" v-show="showPage==='resume'">
      ...
      <div class="resume-item first-resume-item">
        <p class="profile-tip">*迷你校移动端暂不支持修改简历</p>
        ...
        <div class="resume-item-content" v-if="resumeData.basic.name">
          ...
          <div class="basce-span"><span v-if="resumeData.basic.gender">{{resumeData.basic.gender}}</span><span v-if="resumeData.basic.birthday">{{resumeData.basic.birthday}}</span></div>
      ...
      <div class="resume-item" id="resumeCertificate">
        ...
        <div class="pleaseEdit" v-if="!resumeData.certificates">
          <!-- <h2 class="color-red">请编辑</h2> -->
        </div>
        <!-- <div class="resume-item-content edu-item vux-1px-b" v-for="(item,index) in resumeData.certificates" @click="editorCertificate(item, index)" v-if="resumeData.certificates"> -->
        <div class="resume-item-content edu-item vux-1px-b" v-for="(item,index) in resumeData.certificates" v-if="resumeData.certificates">
          <div class="arrow-right">
            <div>{{item.name}}</div>
          </div>
        </div>
      ...
      <div class="resume-item" id="resumeLab">
        <div class="resume-item-title vux-1px-b">
          ...
          <!-- <a href="javascript:;" class="editor-btn iconfont icon-bianji" id="laboratory-editor" v-on:click="editorLab"></a> -->
        </div>
        <div class="resume-item-content">
      ...
      <div class="resume-item" id="resumeLab">
        <div class="resume-item-title vux-1px-b">
          <h2 class="inline-block">发表论文</h2>
          <!-- <a href="javascript:;" class="editor-btn iconfont icon-bianji" id="paper-editor" v-on:click="editorPaper"></a> -->
        </div>
        <div class="resume-item-content">
          <pre v-if="resumeData.paper" class="resume_pre">{{resumeData.paper}}</pre>
        </div>
      </div>
      <!-- 所编书展示 -->
      <div class="resume-item" id="resumeLab">
        <div class="resume-item-title vux-1px-b">
          <h2 class="inline-block">所编书</h2>
          <!-- <a href="javascript:;" class="editor-btn iconfont icon-bianji" id="book-editor" v-on:click="editorBook"></a> -->
        </div>
        <div class="resume-item-content">
          <pre v-if="resumeData.book" class="resume_pre">{{resumeData.book}}</pre>
        </div>
      </div>
      <!-- 其他信息展示 -->
      ...
      <div class="resume-detail-content">
        <div class="resume-detail-view">
          <group class="delBorder">
            ...
            <datetime class="necessary" :class="{fixText: hasStart}" v-model="startDate" format="YYYY-MM" title="入学时间" :min-year="2000" confirm-text="完成" cancel-text="取消" placeholder="请选择入学时间" @on-change="startChange"></datetime>
            <datetime class="necessary" :class="{fixText: hasEnd}" v-model="endDate" format="YYYY-MM" title="毕业时间" :min-year="2000" confirm-text="完成" cancel-text="取消" placeholder="请选择毕业时间" @on-change="endChange"></datetime>
    ...
    <!-- 实验室详细编辑 -->
    <!-- <div id="resumeLanguage" class="resume-laboratory" v-if="showPage==='laboratory'">
    ...
    <!-- 发表论文详细编辑 -->
    <!-- <div id="resumePaper" class="resume-paper" v-if="showPage==='paper'">
      <x-header class="header" :left-options="{backText: '',preventGoBack: true}" @on-click-back="leaveLab">填写发表论文</x-header>
      <div class="resume-detail-content">
        <div class="resume-detail-view">
          <group class="delBorder">
            <x-textarea :max="2000" placeholder="" v-model="resumeData.paper"></x-textarea>
          </group>
        </div>
        <div class="saveBtn-box">
          <button class="saveBtn" @click="saveLab">保存</button>
    ...
    <!-- 所编书详细编辑 -->
    <!-- <div id="resumeBook" class="resume-book" v-if="showPage==='book'">
      <x-header class="header" :left-options="{backText: '',preventGoBack: true}" @on-click-back="leaveLab">填写所编书</x-header>
      <div class="resume-detail-content">
        <div class="resume-detail-view">
          <group class="delBorder">
            <x-textarea :max="2000" placeholder="" v-model="resumeData.book"></x-textarea>
          </group>
        </div>
        <div class="saveBtn-box">
          <button class="saveBtn" @click="saveLab">保存</button>
    ...
    <!-- 证书编辑 -->
    <!-- <div class="resume-skill" v-if="showPage==='certificates'">
...
export default {
  ...
  methods: {
    getData() {
      this.$http.get(`/user/${this.id}/profile`)
        .then((res) => {
          if (res.data.success) {
            let resumeData = res.data.message
            for (let i = 0; i < resumeData.profile.length; i++) {
              switch (resumeData.profile[i].type) {
                case '5001':
                  this.resumeDataWork = resumeData.profile[i].data
                  break
                case '6001':
                  this.resumeDataPractices = resumeData.profile[i].data
                  break
                case '1201':
                  this.resumeDataClub = resumeData.profile[i].data
                  break
                case '3001':
                  this.resumeDataReward = resumeData.profile[i].data
                  break
                case '1401':
                  this.resumeDataLaboratory = resumeData.profile[i].data
                  break
                case '8001':
                  this.resumeDataPaper = resumeData.profile[i].data
                  break
                case '9001':
                  this.resumeDataBook = resumeData.profile[i].data
                  break
              }
            }
            for (let j = 0; j < resumeData.other.length; j++) {
              switch (resumeData.other[j].type) {
                case '7001':
                  this.resumeDataLan.push(JSON.parse(resumeData.other[j].data))
                  break
                case '3002':
                  this.resumeDataCer.push(JSON.parse(resumeData.other[j].data))
                  break
                case '7002':
                  this.resumeDataSkill.push(JSON.parse(resumeData.other[j].data))
                  break
              }
            }
            this.resumeData = {
              id: resumeData.id,
              basic: {
                name: resumeData.realname,
                mobile: resumeData.mobile,
                email: resumeData.email,
                gender: resumeData.gender,
                birthday: resumeData.birthday,
                city: resumeData.province,
                currentCity: resumeData.city
              },
              education: resumeData.education,
              work: this.resumeDataWork,
              practices: this.resumeDataPractices,
              club: this.resumeDataClub,
              reward: this.resumeDataReward,
              laboratory: this.resumeDataLaboratory,
              paper: this.resumeDataPaper,
              book: this.resumeDataBook,
              skill: this.resumeDataLan,
              certificates: this.resumeDataCer,
              language: this.resumeDataSkill
            }
          }
        })
    },
    ...
    saveEdu() {
      if (this.school && this.value1 && this.major && this.startDate && this.endDate) {
        this.showPage = 'resume'
        var deg
        ...
        if (this.addEdu) {
          ...
          this.resumeData.education[this.eduItemIndex].startDate = this.startDate
          this.resumeData.education[this.eduItemIndex].endDate = this.endDate
    ...
    saveCertificate() {
      if (this.certificate) {
        if (this.addCertificate) {
          const tempObj = {}
          tempObj.name = this.certificate
          if (this.resumeData.certificates == null) {
            this.resumeData.certificates = []
          }
          this.resumeData.certificates.push(tempObj)
        } else {
          this.resumeData.certificates[this.certificateIndex].name = this.certificate
        }
      }
      this.showPage = 'resume'
      goAnchor(this.anchor)
      console.log(this.certificate)
      console.log(this.resumeData.certificates)
    },
    deleteCertificate() {
      if (!this.addCertificate) {
        this.resumeData.certificates.splice(this.certificateIndex, 1)
        if (this.resumeData.certificates.length === 0) {
          this.resumeData.certificates = null
        }
      }
      this.showPage = 'resume'
      goAnchor(this.anchor)
